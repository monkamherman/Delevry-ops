...
export interface ClientToServerEvents {
   ...
   */
  admin_subscribe: (callback: (response: { success: boolean }) => void) => void;
}
...
export type TrackingEvent = 
  | { type: 'POSITION_UPDATE'; data: { livreurId: string; position: Position } }
  | { type: 'DELIVERY_ASSIGNED'; data: { deliveryId: string; livreur: Pick<Livreur, 'id' | 'firstName' | 'lastName'> } }
  | { type: 'DELIVERY_STATUS_CHANGED'; data: { deliveryId: string; status: DeliveryStatus } };
