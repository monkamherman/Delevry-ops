...
exports.isValidFrenchSSN = isValidFrenchSSN;
...
function isValidUrl(url, options = {}) {
    if (!url)
        return false;
    const { protocols = ['http', 'https', 'ftp'], requireProtocol = true, requireValidProtocol = true } = options;
    try {
 ...
 */
function isLengthInRange(value, min, max = Number.MAX_SAFE_INTEGER) {
    if (!value)
        return false;
