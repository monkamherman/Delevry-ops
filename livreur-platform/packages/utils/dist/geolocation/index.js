...
exports.calculateWeightedMidpoint = calculateWeightedMidpoint;
/**
 ...
 */
function calculateTravelTime(distance, averageSpeed = 30, trafficFactor = 1.0, stopMinutes = 0) {
    if (distance < 0) {
...
function calculateMidpoint(positions) {
    ...
    const { sumLat, sumLon, count } = positions.reduce((acc, pos) => ({
 ...
 */
function calculateWeightedMidpoint(positions) {
    if (positions.length === 0)
        return null;
    ...
    const { sumLat, sumLon } = weighted.reduce((acc, pos) => ({
